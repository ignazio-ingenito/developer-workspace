...
grep -Fq "github.actor != 'dependabot[bot]'" "$workflow"
...
grep -Fq "bash \"\$RUNNER_TEMP/changelog-branch-update.sh\" status \"\$EVENT_HEAD_SHA\" \"\$HEAD_REF\"" "$workflow"
grep -Fq "bash \"\$RUNNER_TEMP/changelog-branch-update.sh\" push \"\$EVENT_HEAD_SHA\" \"\$HEAD_REF\"" "$workflow"
